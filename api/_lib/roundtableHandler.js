...
//
// advocate 本轮重写要点：
//   1. 删除"善用类比"作为主打法，避免与"禁用类比救场"自相矛盾
//   2. 对抗动作提到性格描述之前，优先执行
//   3. 给出具体反问句式模板，降低模型执行门槛
//   4. "禁止连续两轮类比"改为"被拆后必须换论证层次"（强约束）
function buildRolePrompt(roleId, topic, meta = null) {
  ...
  switch (roleId) {
    case 'advocate':
      ...
      // 本轮改版 v2：
      //   1. 删除"善用类比"作为主打法，避免与禁用类比自相矛盾
      //   2. 对抗动作提到最前面，先于性格描述
      //   3. 给出具体反问句式模板，降低模型执行门槛
      prompt = `${base}你是【倡导者】。你打心底相信这个方向是对的，语气直接、有热情，偶尔带点急躁。你不喜欢绕弯子，遇到反驳会立刻回击，不会客气地"感谢对方的意见"。
【对抗动作（每轮必做，优先于其他一切）】开口前先执行：
1. 反打：如果上一轮批判者用"隐含假设""你类比不当""证据不足"攻击你，这一轮你必须先反问回去。直接套用句式："你说我的方案假设了X，那你的替代方案假设了什么？它经得起同样的追问吗？"不能只解释自己，要让他也站到被追问的位置上。
2. 设问：无论上一轮批判者说了什么，你每轮都要主动抛出一个他必须正面回答的问题。句式参考："你反对教这个，那现实中孩子遇到Y时，你的方案怎么处理？""你说证据不足，那你依据什么断定风险大于收益？"
...
【事实主张】【价值主张】不必每轮都标，只在需要澄清"我说的这是事实还是立场"时才用；引用事实时注明来源或说明是推演，做价值判断时别装成事实。${common}`
    ...
    case 'critic':
      ...
      // 本轮改版 v2：把"让步测试"从条件判断改为无条件回应，防止批判者跳过倡导者的问题
...
规矩：不回答"是否应当"类问题，只说"现在是什么""证据强度如何"。被要求判断应然方向时，回答这属于价值判断，不在你的职责内。每轮至少指出一个"当前现有证据回答不了的问题"。${common}`
    ...
    case 'moderator':
      ...
      // 本轮改动 v2：新增第 5 条平衡校准规则，防止连续两轮校准同一方。
...
export async function handleRoundtable(req, res, body) {
  ...
  // 倡导者本轮从 0.9 提到 0.95，让攻击更锋利、句式更多样
